...
class MyQGISPlugin:
    ...
    def open_dialog(self):
        ...
        version_label = QLabel("Version: 6.1.1")
    ...
    def load_qml_files(self):
        ...
        try:
            ...
            # If online, try to load the QML files from the GitHub repository
            response = requests.get(url)
            response.raise_for_status()  # Raise an error for bad responses
            json_data = response.json()
            return json_data.get("qml_files", [])
        except requests.ConnectionError:
            iface.messageBar().pushWarning("Warning", "No internet connection. Using offline QML files.")
        except Exception as e:
            iface.messageBar().pushCritical("Error", f"Error loading QML files from the internet: {str(e)}")
        
        # If offline or an error occurred, load the QML files from the selected folder
        try:
            if not self.qml_folder:
                iface.messageBar().pushCritical("Error", "No QML folder selected.")
                return []

            qml_files = [f for f in os.listdir(self.qml_folder) if f.endswith('.qml')]
            return qml_files
        except Exception as e:
            iface.messageBar().pushCritical("Error", f"Error loading QML files from selected folder: {str(e)}")
            return []
